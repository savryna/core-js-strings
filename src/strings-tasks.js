...
function removeFirstOccurrences(str, value) {
  const firsIdxSlice = str.indexOf(value);
  return firsIdxSlice >= 0
    ? `${str.slice(0, firsIdxSlice)}${str.slice(firsIdxSlice + value.length)}`
    : str;
}
...
function removeLastOccurrences(str, value) {
  const lasIdx = str.lastIndexOf(value);
  return lasIdx >= 0
    ? `${str.slice(0, lasIdx)}${str.slice(lasIdx + value.length)}`
    : str;
}
...
function sumOfCodes(str) {
  ...
  return [...str].reduce((acc, cur) => acc + cur.charCodeAt(), 0);
}
...
function orderAlphabetically(str) {
  return [...str].sort((a, b) => a.codePointAt() - b.codePointAt()).join('');
}
...
function countVowels(str) {
  return (
    str.length - str.replace(/['a', 'e', 'i', 'o', 'u', 'y']/gi, '').length
  );
}
...
function isPalindrome(str) {
  const arr = [...str.toLowerCase().replace(/\W/gi, '')];
...
function extractNameFromTemplate(value) {
  return value.replace(/Hello/, '').replace(/\W/gi, ' ').trim();
}
